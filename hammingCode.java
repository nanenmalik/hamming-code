...
public class HammingCode {
    ...
    static int getParityBit(int returnData[], int pow) {  
        ...
        for(int i = 0; i < size; i++) {  
            ...
            if(returnData[i] != 2) {  
                ...
                // convert the k to binary
                String str = Integer.toBinaryString(k);  
              
                // find bit at the location of parityBit 
    ...
    // create receiveData() method that receives the data sent by the sender and checks for error 
    static void receiveData(int data[], int parityBits) {  
          
        // declare variables  
        int pow;  
        int size = data.length;       
        // declare an array for storing the parities  
        int parityArray[] = new int[parityBits];  
        // declare a string for storing the error location 
        String errorLoc = new String();  
        // use for loop to extract the parity bits
        ...
        // convert the binary value stored in errorLoc to decimal
        int finalLoc = Integer.parseInt(errorLoc, 2);  
        // if finalLoc is not equal to 0, then error is detected 
        if(finalLoc != 0) {  
            System.out.println("Error is found at location " + finalLoc + ".");  
            data[finalLoc - 1] = (data[finalLoc - 1] + 1) % 2;  
            System.out.println("The corrected code:");  
            for(int i = 0; i < size; i++) {  
        ...
        // print the data sent by the sender  
